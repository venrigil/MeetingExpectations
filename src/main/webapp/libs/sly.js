/*! Sly v1.0rc2 <http://sly.digitarald.com> - (C) 2009 Harald Kirschner <http:
...
proto.search = function(context, results, options) {
	...
	if (!context) {
		...
		if (typeof(context) == 'string') {
			...
			iterate = true;
		} else if (Object.prototype.toString.call(context) == '[object Array]' || (typeof(context.length) == 'number' && context.item)) { 
			var filtered = [];
	...
	for (var i = 0, selector; (selector = parsed[i]); i++) {
		...
		if (selector.first) {
			...
			else if (selector.combinator) nodes = [context]; 
		}
...
proto.compute = function(selector) {
	...
	if (classes.length > 0) {
		...
		} else if (!search && classes.length == 1) { 
