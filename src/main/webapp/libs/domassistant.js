// Developed by Robert Nyman/DOMAssistant team, code/licensing: http://domassistant.googlecode.com/, documentation: http:
			...
			DOMAssistant.cssByXpath = function (cssRule) {
				...
				for (var i=0; (currentRule=cssRules[i]); i++) {
					...
					for (var j=0, jl=cssSelectors.length; j<jl; j++) {
						...
						xPathExpression +=
							(splitRule.tagRelation? ({ ">": "/", "+": "/following-sibling::*[1]/self::", "~": "/following-sibling::" }[splitRule.tagRelation] || "") : ((j > 0 && regex.relation.test(cssSelectors[j-1]))? splitRule.tag : ("
...
DOMAssistant.Events = function () {
	var handler,
		...
		createEvent = function (e, type, target) {
			...
			if (event.target && 3 === event.target.nodeType) { 
...
DOMAssistant.DOMLoad = function () {
	...
	/*@cc_on
	@if (@_win32 || @_win64)
		document.write("<script id=\"ieScriptLoad\" defer src=\"
