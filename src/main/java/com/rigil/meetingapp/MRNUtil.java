...
public class MRNUtil {
  ...
  public static String writeJSON(Iterable<Entity> entities, String childKind, String fkName) {
    ...
    for (Entity result : entities) {
      Map<String, Object> properties = result.getProperties();
      sb.append("{");
      if (result.getKey().getName() == null) { sb.append("\"name\" : \"" + result.getKey().getId() + "\","); }
      ...
      for (Entity en : child) {
        for (String key : en.getProperties().keySet()) { sb.append("\"" + key + "\" : \"" + en.getProperties().get(key)+ "\","); } }
      sb.deleteCharAt(sb.lastIndexOf(","));
      sb.append("},");
      i++; }
    if (i > 0) { sb.deleteCharAt(sb.lastIndexOf(","));  }
    sb.append("]}");
    return sb.toString(); }
  ...
  public static String writeEntityJSON( Entity entity, String childKind, String fkName) {
	    ...
	    sbmreport.append("{\"data\": [");
	      ...
	      for (Entity en : child)
	      { 
	    	  sbagl.append("{"); for (String childkey : en.getProperties().keySet()) 
	    	  {
	    ...
	    if (i > 0) 
	    {
	    	logger.log(Level.INFO,""+i+"  ");sb.deleteCharAt(sb.lastIndexOf(",")); 
	    }
