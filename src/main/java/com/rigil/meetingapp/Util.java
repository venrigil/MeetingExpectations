...
public class Util {
  ...
  public static Iterable<Entity> listChildrenSearch(String kind, String searchBy, String searchFor, String ancestor) {
  ...
  public static String writeJSON(Iterable<Entity> entities, String childKind, String fkName) {
    ...
    int i = 0;
    sbhtml.append("<html><head></head><body><table border=2><tr>");
    sb.append("{\"data\": [");
    for (Entity result : entities) {
      Map<String, Object> properties = result.getProperties();
      sbhtml.append("<td>");
      ...
      {
        sb.append("\"name\" : \"" + result.getKey().getId() + "\",");
      sbhtml.append("\"name\" : \"" + result.getKey().getId() + "\",<br>");
      
      }
      else
      {
        sb.append("\"name\" : \"" + result.getKey().getName() + "\",");
        sbhtml.append("\"name\" : \"" + result.getKey().getName() + "\",<br>");
        
      }
      for (String key : properties.keySet()) {
        sb.append("\"" + key + "\" : \"" + properties.get(key) + "\",");
        sbhtml.append("\"" + key + "\" : \"" + properties.get(key) + "\",<br>");

        if(key.equals("participantsString"))
        {
        	...
        	String pstr = String.valueOf(properties.get(key));
        	  ...
        	  for(int psi =0; psi < ptemp.length ; psi++)
        	  {
        		  ...
        		  Iterable<Entity> pschild = listEntities("Directory", "contactID",ptemp[psi]);
        		  
        		  for (Entity psen : pschild) {
        	    	  
        	        for (String pskey : psen.getProperties().keySet()) {
        	        	
        	          sb.append("\"" + pskey + "\" : \"" + psen.getProperties().get(pskey)+ "\",");
        	          sbhtml.append("\"" + pskey + "\" : \"" + psen.getProperties().get(pskey)+ "\",<br>");
        	        }
        	      }
      ...
      Iterable<Entity> child = listEntities(childKind, fkName,String.valueOf(result.getKey().getName()));
      ...
      for (Entity en : child) {
        ...
        for (String key : en.getProperties().keySet()) {
          ...
          sbhtml.append("\"" + key + "\" : \"" + en.getProperties().get(key)+ "\",<br>");
        }
      }
     
      sb.deleteCharAt(sb.lastIndexOf(","));
      sbhtml.deleteCharAt(sb.lastIndexOf(","));
      sb.append("},");
      sbhtml.append("</td><br>");
      i++;
    ...
    sb.append("]}");
    sbhtml.append("</tr></table></body></html>");
    logger.log(Level.INFO,""+sbhtml+"  ");
  ...
  public static String writeEntityJSON( Entity entity, String childKind, String fkName) {
	    ...
	    sb.append("{\"data\": [");
	      ...
	      Iterable<Entity> child = listEntities(childKind, fkName,String.valueOf(entity.getKey().getName()));
	      ...
	      for (Entity en : child) {
	        ...
	        for (String childkey : en.getProperties().keySet()) {
	          sb.append("\"" + childkey + "\" : \"" + en.getProperties().get(childkey)+ "\",");
	        }
	      }
